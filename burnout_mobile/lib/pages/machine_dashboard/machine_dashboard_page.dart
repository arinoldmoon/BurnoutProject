...
import '../../grpc/google/protobuf/empty.pb.dart';
...
class _MachineDashboardPageState extends State<MachineDashboardPage>
  ...
  static final channel = ClientChannel(
    '127.0.0.1', // Use your IP address where the server is running
    port: 5000,
  ...
  void callGrpc() async {
    ...
    ProtoPatternList patternListCount =
        await patternStub.getPatternList(Empty());
    callMonitor();
  }
    ...
    if (mounted) {
      setState(() {
        machineData = MockMachinePayload(
          ...
          machineModelYear: '',
          machineNetworkConnection: MachineNetworkConnection(
              machineNetworkConnectionDNS: '',
              machineNetworkConnectionEthernet: '',
              machineNetworkConnectionGateway: '',
              machineWifiConnection: MachineWifiConnection(
